# Convenciones para sesiones de Codex en Trama

Este archivo lo lee Codex automáticamente al entrar al proyecto. Acá viven solo **las reglas que no se pueden romper** y un **índice** a los documentos detallados. Lo específico (design tokens, LLM, dominios, etc.) se splitteó en `docs/conventions/*` en G1.
...
| [**storage-boundaries.md**](docs/conventions/storage-boundaries.md)           | Adapter de storage, manifest de blobs, guardrails y plan de migración futura                        |
...
| [**developer-quality-gates.md**](docs/conventions/developer-quality-gates.md) | Knip, dependency-cruiser, falsos positivos y excepciones justificadas                               |
...
| Doc                            | Cubre                                                                                                                                                            |
| ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `docs/arquitectura/mapa.html`  | Mapa interactivo de la forma actual del sistema (73 nodos, 11 flujos); `mapa.json` es el mismo grafo para agentes. Verificado en CI por `check:architecture-map` |
...
Si descubrís un pattern, regla o "no toques esto" relevante para futuras sesiones de Codex:
...
El objetivo: que `AGENTS.md` se mantenga corto (< 100 líneas). Si está creciendo, splitea.
