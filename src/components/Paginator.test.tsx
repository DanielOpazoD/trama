...
describe('<Paginator />', () => {
  it('shows "más atrás ↓" when hasNext=true and not loading', () => {
    render(
      <Paginator hasNext loading={false} onLoadMore={() => {}} />,
    )
    expect(
      screen.getByRole('button', { name: /más atrás/i }),
    ).toBeInTheDocument()
  })

  it('shows "cargando más…" when loading=true', () => {
    render(
      <Paginator hasNext loading onLoadMore={() => {}} />,
    )
  ...
  it('calls onLoadMore when "más atrás" is clicked', () => {
    ...
    render(
      <Paginator hasNext loading={false} onLoadMore={onLoadMore} />,
    )
  ...
  it('disables the load-more button while loading', () => {
    ...
    expect(
      screen.getByRole('button', { name: /cargando más/i }),
    ).toBeDisabled()
  })
  ...
  it('shows end-mark when no more pages + showEndMark=true (default)', () => {
    ...
    expect(
      screen.queryByRole('button', { name: /más atrás/i }),
