...
describe('<EditorToolbar />', () => {
  ...
  it('expone la herramienta X también al diseñar planillas', () => {
    setup({ context: 'templateDesign' })
    expect(
      screen.getByRole('button', { name: /marca X para casilleros/i }),
    ).toBeInTheDocument()
  })
  ...
  it('agrupa fuente/color/estilo de texto en un menú y formas en otro', () => {
    const p = setup({ hasDuplicableSelection: true, hasSelection: true })

    // Los controles de estilo no se ven hasta abrir el menú "Texto".
    expect(
      screen.queryByRole('menuitemradio', { name: 'Fuente Serif' }),
    ).not.toBeInTheDocument()
    expect(
      screen.queryByRole('menuitemradio', { name: 'Color Amarillo' }),
    ).not.toBeInTheDocument()
    expect(
      screen.queryByLabelText('Rotación del texto: aumentar'),
    ).not.toBeInTheDocument()

    // Un solo menú "Texto" agrupa fuente, color, tamaño, rotación, etc.
    ...
    // Formas sigue en su menú; Resaltar ahora es una herramienta directa.
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Herramienta Resaltar' }))
