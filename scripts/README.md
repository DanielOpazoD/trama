...
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| Antes de commit         | `npm run format:check`, test focalizado, `npm run check:script-registry`, `npm run check:knip`, `npm run check:dead-code`, `npm run check:architecture`, `npm run check:dependency-cruiser`             | Feedback rapido antes de push.                                                              |
...
| Cambios arquitectura    | `check:architecture`, `check:dependency-cruiser`, `check:knip`, `check:dead-code`, `report:quality-gates`, `check:frontend-boundaries`, `check:storage-boundaries`                                      | Fronteras importables, ciclos, dead code y dependencias declaradas.                         |
| Cambios auth/multiuser  | `check:legacy-fallback`, `check:auth-rls-contracts`, `check:legacy-identity-contracts`, `check:user-id-writes`, `legacy-data-reassignment:dry-run`, `check:operational-observability`                   | Complementar con smoke si toca aislamiento real; usar `--out-dir` para artifacts read-only. |
| Cambios DB/migraciones  | `check:migration-duplicates`, `check:hard-delete-allowlist`, `check:storage-boundaries`, `storage-assets:report`, `check:legacy-identity-schema`, `scripts/apply-migrations.sh`, `check:cte-regression` | No editar migraciones aplicadas; reportes read-only no reemplazan migraciones.              |
...
| `ci`        | Registry, dead code, grafo de imports y ratchets de quality gates.      | `check-script-registry`, `check:knip`, `check:dead-code`, `report:quality-gates`, `check:architecture`, `check:dependency-cruiser`, `check-structure-ratchets`, `verify-pr-stack`. |
| `database`  | Migraciones, soft-delete, CTEs atomicos, storage y Postgres real.       | `apply-migrations`, `check:migration-duplicates`, `check:hard-delete-allowlist`, `check:storage-boundaries`, `storage-assets:report`, `check:legacy-identity-schema`.              |
...
| `multiuser` | Cutover, smokes productivos, reportes y observabilidad de aislamiento.  | `cutover-*`, `legacy-data-reassignment-dry-run`, `run-legacy-identity-smoke`, `smoke-isolation`, `multiuser-production-report`.                                                    |
