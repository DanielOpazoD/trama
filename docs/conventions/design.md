...
- `:focus-visible` global con outline azul (salvia en `.pdf-studio`); es la convención de foco por defecto: los componentes no hornean un anillo propio (lo duplicaría) ni lo suprimen con `focus:outline-none` sin reemplazo (lo borraría). Cuando un anillo SÍ está justificado (tarjetas/miniaturas redondeadas, controles sobre imagen, contenedores con `overflow-hidden`), el estándar es el token **`.focus-ring`** / **`.focus-ring-inset`** (`src/index.css`) — un único anillo con el color del outline global (salvia dentro de `.pdf-studio`), no utilidades `focus-visible:ring-*` ad-hoc. Todo esto lo gobierna `check:focus-ring` (baseline 0: el foco horneado o usa el token o cae en el outline global). EXEMPT: inputs cuyo indicador es un wrapper con `:focus-within`/cambio de borde, y contenedores de modal con `tabIndex={-1}`
