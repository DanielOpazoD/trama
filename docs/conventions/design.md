...
- **Sistema de anillos** (todos con el mismo lenguaje de `box-shadow`, en `src/index.css`): además del foco (`.focus-ring`/`.focus-ring-inset`), `.focus-ring-within` ilumina un wrapper al enfocar un hijo (inputs transparentes, reemplaza `focus-within:border-*` ad-hoc), y la SELECCIÓN (estado "ítem elegido", distinto del foco) usa `.selection-ring` (salvia) / `.selection-ring-gold` (oro en momentos/recortes) en vez de `boxShadow` inline disperso. El foco le gana a la selección por especificidad, así que un ítem elegido y enfocado muestra el foco. Verificado en runtime por `e2e/focus-ring.spec.ts` (el anillo se dibuja por teclado, adapta color en dark, y un control sin token cae en el outline global) y con regresión visual opt-in en `e2e/focus-ring-visual.spec.ts` (`npm run e2e:focus-visual`; snapshots `-darwin`; corren en el workflow `pdf-visual` sobre macOS, junto a los de `pdf-studio-visual`)
