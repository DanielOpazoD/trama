...
- **Smoke real de espacio compartido:** `e2e/momentos-shared-space.spec.ts` prueba A sube una foto privada, B no puede leer el blob antes de aceptar la invitación, B sí puede leerlo después, `editor` puede editar, `viewer` no edita pero comenta/reacciona, el dueño borra comentarios y la revocación vuelve el blob a 404. Requiere `E2E_BASE_URL`, `E2E_USER_A_TOKEN`, `E2E_USER_B_TOKEN` y `E2E_USER_B_EMAIL`; sin eso se skippea. `npm run e2e:multiuser` genera esos tokens desde Clerk si existen `CLERK_SECRET_KEY`, `E2E_USER_A_ID` y `E2E_USER_B_ID`.
