...
- **Editor de PDF (sección `pdf` del mundo Notas).** Herramienta **100% client-side** (nada se sube) para combinar **PDFs + imágenes** en un documento a nivel de **página** (estilo iLovePDF): cada PDF se expande en sus páginas y cada imagen es una página; se ven como miniaturas reordenables/borrables y se guardan como un PDF nuevo. Se integra con `useModuleVisibility` (ocultable) y el comando `#pdf`. Por área:
  - **Arquitectura.** Modelo **puro y testeado** (`model.ts`) separado de los **bordes browser-only** (excluidos del coverage): `pdfRender.ts` (miniaturas pdf.js), `assemble.ts` (ensamblado pdf-lib), `persistence.ts`, `printPdf.ts` y la UI `components/notas/pdfStudio/**`. `editorGeometry.ts` aísla la **matemática pura del editor** (pantalla→página, fit, rectángulo de arrastre — testeada). `pdf-lib` (con `@pdf-lib/fontkit`) y `pdfjs-dist` (~1MB + worker) se cargan **PEREZOSO** (`React.lazy` + `await import()` → chunks `vendor-pdf-lib`/`pdf` aparte, el bundle `index` no se toca).
