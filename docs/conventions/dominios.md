...
- **Anexos de Notas/Prompts** viven en Netlify Blobs vía endpoints backend (`notas-attachments-*`), nunca desde cliente. Cada upload/list/download valida `user_id` y dueño activo (`note` o `prompt` con `deleted_at IS NULL`). Los uploads nuevos cifran bytes en cliente antes de subir (`.tramaenc`); el backend conserva solo metadata de archivo y nunca recibe la key local. Al borrar una nota o prompt, sus anexos se soft-deletean.
