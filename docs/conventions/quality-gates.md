...
| API/backend      | `check:runtime-api-routes`, `check:api-request-contracts`, `check:api-error-shape`, `check:backend-domain-services`, `check:client-api-contracts` | Wrappers Netlify, parsing, shape de errores, servicios y uso cliente de API. | Arreglar frontera o documentar excepcion en el script correspondiente.  |
...
| Estructura       | `check:structure-ratchets`, `check:frontend-boundaries`                                                                                           | Hotspots y fronteras frontend.                                               | Extraer responsabilidades; el ratchet no se sube (ADR-0016).            |
